...
export function RiderHeader({ collapsed, setCollapsed, notifications, onSignOut }: RiderHeaderProps) {
  ...
  useEffect(() => {
      // Get rider data from API
      const fetchRiderData = async () => {
        try {
          const token = localStorage.getItem(STORAGE_KEYS.RIDER_TOKEN) || localStorage.getItem("riderToken")
          if (token) {
              ...
              <Button variant="ghost" className="relative h-8 w-8 rounded-full">
                  ...
                  <AvatarFallback className="bg-secondary text-white  cursor-pointer">
                    { (rider?.firstName || rider?.lastName)
                      ? `${(rider?.firstName?.[0] ?? "")}${(rider?.lastName?.[0] ?? "")}`.replace(/\s+/g, "").toUpperCase().slice(0,2)
                      : "R" }
                  </AvatarFallback>
                </Avatar>
              </Button>
            </DropdownMenuTrigger>
            <DropdownMenuContent className="w-56 border-none p-2 bg-white" align="end" forceMount>
              ...
              <DropdownMenuItem asChild className="hover:bg-primary hover:text-white text-foreground cursor-pointer">
              ...
              <DropdownMenuSeparator className="bg-primary/50 " />
              <DropdownMenuItem className="hover:bg-primary hover:text-white text-foreground cursor-pointer" onClick={onSignOut}>
