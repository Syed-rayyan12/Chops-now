            ...
            <div className="container mx-auto px-24 max-2xl:scale-[0.98] max-2xl:px-0 max-sm:px-8 max-sm:py-20 pt-18 z-20 text-center text-white">
                <div className="flex max-sm:flex-col max-lg:flex-col max-xl:flex-col max-xl:px-44 max-lg:px-6  max-sm:gap-6 gap-8 justify-around items-center">
                    <div className="flex flex-col gap-10 z-3 max-lg:w-full max-xl:w-full">
                        {[
                            { img: "/who-4.svg", text: "Authentic, Home-Style Recipes" },
                            { img: "/who-1.svg", text: "Empowering Our Food Community" },
                            { img: "/who-2.svg", text: "Support for Home Cooks & Chefs" },
                            { img: "/who-3.svg", text: "The True Taste Experience" },
                        ].map((item, index) => (
                            <div
                                key={index}
                                className="relative bg-white flex items-center justify-start px-5 gap-6 font-space-grotesk font-bold w-82 h-22 max-lg:w-full max-xl:w-full rounded-[40px]"
                            >
                                {/* Custom border */}
                                ...
                                {/* Text */}
                                <span className="text-foreground text-[15px] text-left">{item.text}</span>
                            </div>
                        ))}
                    </div>
                    ...
                    <div className='text-left max-lg:mt-8'>
                        <h2 className='font-fredoka-one text-5xl mb-5 max-sm:text-2xl text-foreground font-extrabold'>Bringing Authentic Taste to Your Home</h2>
                        <div className="bg-secondary h-1 w-24 mb-5"></div>
                        <p className='text-[16px] text-foreground font-ubuntu'>ChopNow is a proud black-owned food delivery platform UK residents can trust for real cultural flavour.
                            We celebrate heritage through authentic African meals UK families love and fresh Caribbean food
                            delivery UK communities enjoy. Supporting local chefs and home cooks, we make it easy to connect,
                            order, and enjoy. Simply order via our website and taste home today!</p>
